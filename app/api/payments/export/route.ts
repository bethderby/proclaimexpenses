...
export async function GET(req:NextRequest){
 ...
 if(!user.isAdmin){const allowed=run.expenses.some(e=>e.team.members?.some?.((m:any)=>m.userId===user.id&&m.role==='APPROVER'));if(!allowed){const own=await prisma.expense.findFirst({where:{paymentRunId:runId,team:{members:{some:{userId:user.id,role:'APPROVER'}}}}});if(!own)return NextResponse.json({error:'Forbidden.'},{status:403});}}
