...
export async function GET(req:NextRequest){
 ...
 if(!user.isAdmin){const approverEmail=(user.email??'').toLowerCase();const allowed=run.expenses.some(e=>e.team.approverEmails.some((email:string)=>email.toLowerCase()===approverEmail));if(!allowed)return NextResponse.json({error:'Forbidden.'},{status:403});}
