...
export async function GET(req:NextRequest){
 ...
 if(!user.isAdmin){
  ...
  exportExpenses = run.expenses.filter(e=>e.team.approverEmails.some((email:string)=>email.toLowerCase()===approverEmail));
  if(!exportExpenses.length)return NextResponse.json({error:'Forbidden.'},{status:403});
 }
 const escape=(v:string)=>`"${String(v??'').replace(/"/g,'""')}"`;
 const rows=[['Account name','Sort code','Account number','Amount','Reference','Employee','Team'],...exportExpenses.map(e=>[decryptBankDetail(e.user.bankAccountName),decryptBankDetail(e.user.bankSortCode),decryptBankDetail(e.user.bankAccountNumber),e.amount.toFixed(2),e.paymentReference||run.reference,e.user.name||e.user.email||'',e.team.name])];
