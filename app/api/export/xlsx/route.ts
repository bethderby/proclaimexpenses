...
export async function GET(req:NextRequest){
 ...
 const rows=expenses.map(e=>({Date:e.date.toLocaleDateString('en-GB'),Team:e.team.name,Employee:e.user.name??'',Email:e.user.email??'',Description:e.description,Amount:e.amount,'Purchase status':e.purchaseStatus,'Payment timing':e.paymentTiming,Status:e.status,'Has receipt':e.receiptUrl?'Yes':'No','Payment status':e.paymentStatus,'Payment run':e.paymentReference??''}));
 ...
 const wb=XLSX.utils.book_new();const ws=XLSX.utils.json_to_sheet(rows);const pws=XLSX.utils.json_to_sheet(paymentRows);ws['!cols']=[{wch:12},{wch:18},{wch:20},{wch:28},{wch:35},{wch:12},{wch:24},{wch:28},{wch:12},{wch:18},{wch:22}];pws['!cols']=[{wch:24},{wch:12},{wch:16},{wch:12},{wch:22},{wch:24},{wch:20}];XLSX.utils.book_append_sheet(wb,ws,'Expenses');XLSX.utils.book_append_sheet(wb,pws,'Payment instructions');
 const buffer=XLSX.write(wb,{type:'buffer',bookType:'xlsx'});return new NextResponse(new Uint8Array(buffer),{headers:{'Content-Type':'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet','Content-Disposition':`attachment; filename="${startValue&&endValue?`Proclaim-expenses-${startValue}-to-${endValue}`:'Proclaim-expenses-export'}.xlsx"`} });
}
