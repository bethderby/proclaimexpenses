...
export default async function OutstandingPage(){
 ...
 const [teams,missingReceipts,readyToPay,paymentIssues]=await Promise.all([
  ...
  prisma.expense.findMany({where:{status:'PAYMENT_FAILED'},include:{user:true,team:true},orderBy:{submittedAt:'desc'}}),
 ]);
 ...
 return <div className="page-stack">
  <header className="page-header"><div><p className="page-eyebrow">Overview</p><h1 className="page-title">Outstanding</h1><p className="page-description">Everything across the business that still needs to move: receipts owed, approved expenses waiting on payment, and anything that needs attention. Nothing here needs your action directly &mdash; it's a place to keep an eye on what's outstanding and nudge people if needed.</p></div></header>
  ...
  <section>
   <div className="mb-4"><p className="panel-kicker">Export</p><h2 className="font-semibold text-slate-950">Activity log</h2><p className="mt-1 text-sm text-slate-500">Export everything that happened for a team and date range &mdash; including rejections, cancellations and failed payments.</p></div>
   <ActivityLogExport teams={teams.map(t=>({id:t.id,name:t.name}))}/>
  </section>
 </div>;
}
