    ...
    <>
      <aside className="hidden w-72 shrink-0 border-r border-slate-200 bg-white lg:flex lg:min-h-screen lg:flex-col">
        <div className="border-b border-slate-100 px-6 py-6">
          <div className="flex items-center gap-3"><Image src="/favicon.png" alt="Proclaim" width={40} height={40} className="h-10 w-10 rounded-xl object-cover" /><div><p className="font-semibold text-slate-950">Proclaim Expenses</p><p className="text-xs text-slate-500">Expense Management</p></div></div>
        </div>
      ...
      <div className="sticky top-0 z-40 flex items-center justify-between border-b border-slate-200 bg-white/95 px-4 py-3 backdrop-blur lg:hidden">
        <div className="flex items-center gap-2.5"><Image src="/favicon.png" alt="Proclaim" width={32} height={32} className="h-8 w-8 rounded-lg object-cover" /><p className="font-semibold text-slate-950">Proclaim Expenses</p></div>
      ...
      <div className={`fixed inset-0 z-50 lg:hidden ${open ? '' : 'pointer-events-none'}`}>
        ...
        <aside className={`absolute right-0 top-0 flex h-full w-[82%] max-w-xs flex-col bg-white shadow-xl transition-transform duration-200 ${open ? 'translate-x-0' : 'translate-x-full'}`}>
          <div className="flex items-center justify-between border-b border-slate-100 px-5 py-5">
            <div className="flex items-center gap-3"><Image src="/favicon.png" alt="Proclaim" width={36} height={36} className="h-9 w-9 rounded-xl object-cover" /><div><p className="font-semibold text-slate-950">Proclaim Expenses</p><p className="text-xs text-slate-500">Expense Management</p></div></div>
