    ...
    <>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-40 border-t border-slate-200 bg-white/95 px-2 py-2 backdrop-blur lg:hidden">
        <div className="mx-auto flex max-w-xl gap-1 overflow-x-auto">
          {items.map((it)=>{const Icon=it.icon; const active=pathname===it.href || pathname.startsWith(it.href+'/'); return <Link key={it.href} href={it.href} className={`relative flex w-[76px] shrink-0 flex-col items-center gap-1 rounded-xl px-1 py-2 text-[10px] font-medium ${active?'bg-slate-950 text-white':'text-slate-500'}`}><Icon size={18}/><span className="max-w-full truncate">{it.label}</span>{'badge' in it && !!it.badge && <span className="absolute right-3 top-1 h-2 w-2 rounded-full bg-emerald-500"/>}</Link>})}
        </div>
      </nav>
    </>
  );
