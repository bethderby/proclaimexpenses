...
export default function EditExpenseForm({
  ...
  expense: { id: string; date: string; description: string; amount: number; teamId: string };
  ...
  return (
    <form
      action={(fd) => startTransition(async () => { setError(''); try { await updateExpense(fd); onDone(); } catch (e: any) { setError(e?.message || 'Could not update expense.'); } })}
    ...
    >
      ...
      <div className="flex gap-2 sm:col-span-2">
        <button disabled={pending} className="rounded-lg bg-slate-950 px-4 py-2 text-xs font-semibold text-white transition active:scale-[.97] disabled:cursor-wait disabled:bg-emerald-700">{pending ? 'Saving…' : 'Save changes'}</button>
        <button type="button" onClick={onDone} className="rounded-lg border border-slate-200 px-4 py-2 text-xs font-semibold text-slate-600 transition hover:bg-slate-50 active:scale-[.97]">Cancel</button>
      </div>
    </form>
  );
}
