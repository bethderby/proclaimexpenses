...
export default function ApprovalRow({ expense }: { expense: { id:string; description:string; amount:number; date:string; userName:string; teamName:string; purchaseStatus:string; paymentTiming:string; receiptUrl:string|null; status:string } }) {
 ...
 return <div className="panel p-5">
   ...
   <div className="mt-4 flex flex-wrap gap-2"><span className="inline-flex items-center gap-1.5 rounded-full bg-slate-50 px-2.5 py-1 text-xs font-medium text-slate-600"><WalletCards size={13}/>{mode}</span>{expense.receiptUrl && <span className="inline-flex items-center gap-1.5 rounded-full bg-[#EAF6F1] px-2.5 py-1 text-xs font-medium text-[#146650]"><ReceiptText size={13}/>Receipt attached</span>}</div>
   {expense.receiptUrl && <a href={`/api/receipts/${expense.id}`} target="_blank" rel="noreferrer" className="mt-3 inline-block text-xs font-semibold text-[#146650] hover:underline">View receipt</a>}
   <input value={note} onChange={e=>setNote(e.target.value)} placeholder="Optional note for the requester" className="mt-4 w-full rounded-xl border border-stone-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#146650]" />
   <div className="mt-3 flex gap-2"><button disabled={pending} onClick={()=>decide('APPROVED')} className="flex items-center gap-1.5 rounded-xl bg-[#146650] px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-60"><CheckCircle2 size={15}/>{action==='APPROVED'?'Approving…':'Approve'}</button><button disabled={pending} onClick={()=>decide('REJECTED')} className="flex items-center gap-1.5 rounded-xl border border-rose-300 px-4 py-2.5 text-sm font-semibold text-rose-700 disabled:opacity-60"><XCircle size={15}/>{action==='REJECTED'?'Rejecting…':'Reject'}</button></div>
 </div>;
}
