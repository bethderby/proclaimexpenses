...
export default function BudgetCharts({barData,trendData,pieData}:{barData:{name:string;Spent:number}[];trendData:{month:string;Spend:number}[];pieData:{name:string;value:number}[]}){
 return <div className="grid gap-6 lg:grid-cols-2">
   ...
   <div className="panel p-5">
     ...
     {pieData.length===0?<p className="py-20 text-center text-sm text-slate-400">No expenses logged yet.</p>:<ResponsiveContainer width="100%" height={280}><PieChart><Pie data={pieData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={95} innerRadius={45} paddingAngle={2}>{pieData.map((_,i)=><Cell key={i} fill={`hsl(${i*47}, 62%, 45%)`}/>)}</Pie><Tooltip formatter={(v)=>fmt(Number(v))}/><Legend/></PieChart></ResponsiveContainer>}
   </div>
