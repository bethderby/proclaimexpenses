         ...
         <tbody className="divide-y divide-slate-100">
           {teams.map(t=>{
             ...
             return <tr key={t.id} className="transition hover:bg-slate-50">
               ...
               <td className="px-5 py-5 align-top"><div className="flex justify-end gap-1"><button type="button" onClick={()=>{setEditing(t);setOpen(true)}} aria-label={`Edit ${t.name}`} title="Edit team" className="rounded-lg p-2 text-stone-600 hover:bg-slate-100 hover:text-slate-900"><Pencil size={17}/></button><form action={deleteTeam} onSubmit={e=>{if(!window.confirm(`Archive the ${t.name} team? Its expenses and history will be kept, but the team will no longer be available for new expenses.`))e.preventDefault()}}><input type="hidden" name="teamId" value={t.id}/><FormButton pendingLabel="Archiving…" className="rounded-lg p-2 text-slate-500 hover:bg-amber-50 hover:text-amber-700"><Archive size={17}/></FormButton></form></div></td>
             </tr>;
           })}
         </tbody>
       </table>
