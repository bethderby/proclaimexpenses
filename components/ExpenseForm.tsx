...
export default function ExpenseForm({ teams, hasBankDetails }: { teams: { id: string; name: string }[]; hasBankDetails: boolean }) {
  ...
  return <>
    <form action={submitExpense} onSubmit={handleSubmit} className="space-y-5 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
      ...
      <div><label className="block text-sm font-medium text-stone-700 mb-1">Team</label><select name="teamId" required className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#FDCF37]">{teams.map(t=><option key={t.id} value={t.id}>{t.name}</option>)}</select></div>
      <div><label className="block text-sm font-medium text-stone-700 mb-1">What's this expense for?</label><input name="description" required className="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#FDCF37]" placeholder="e.g. Train tickets for conference"/></div>
      <div><label className="block text-sm font-medium text-stone-700 mb-1">Amount</label><div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-stone-400">£</span><input type="number" step="0.01" min="0.01" name="amount" required className="h-10 w-full rounded-xl border border-stone-300 pl-7 pr-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#FDCF37]"/></div></div>

      <div>
        <p className="mb-2 block text-sm font-medium text-stone-700">Have you already bought this?</p>
        <div className="grid gap-2 sm:grid-cols-2">
          <button type="button" onClick={()=>setPurchaseStatus('ALREADY_PURCHASED')} className={`rounded-xl border p-3 text-left ${alreadyBought?'border-[#FDCF37] bg-[#FFF8E1] ring-1 ring-[#FDCF37]':'border-slate-200 hover:bg-slate-50'}`}><p className="text-sm font-semibold text-slate-900">Yes, I've already bought it</p><p className="mt-0.5 text-xs text-slate-500">Receipt required now.</p></button>
          <button type="button" onClick={()=>setPurchaseStatus('NOT_PURCHASED')} className={`rounded-xl border p-3 text-left ${!alreadyBought?'border-[#FDCF37] bg-[#FFF8E1] ring-1 ring-[#FDCF37]':'border-slate-200 hover:bg-slate-50'}`}><p className="text-sm font-semibold text-slate-900">No, I haven't bought it</p><p className="mt-0.5 text-xs text-slate-500">Request a cash advance - we'll pay you first.</p></button>
        </div>
      </div>
