    ...
    <form action={submitExpense} onSubmit={handleSubmit} className="panel space-y-5 p-5 sm:p-6">
      ...
      <div>
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          <button type="button" onClick={()=>setPurchaseStatus('ALREADY_PURCHASED')} className={`rounded-xl border p-3 text-left ${alreadyBought?'border-[#146650] bg-[#EAF6F1] ring-1 ring-[#146650]':'border-slate-200 hover:bg-slate-50'}`}><p className="text-sm font-semibold text-slate-900">Yes, I've already bought it</p><p className="mt-0.5 text-xs text-slate-500">Receipt required now.</p></button>
          <button type="button" onClick={()=>setPurchaseStatus('NOT_PURCHASED')} className={`rounded-xl border p-3 text-left ${!alreadyBought?'border-[#146650] bg-[#EAF6F1] ring-1 ring-[#146650]':'border-slate-200 hover:bg-slate-50'}`}><p className="text-sm font-semibold text-slate-900">No, I haven't bought it</p><p className="mt-0.5 text-xs text-slate-500">Request a cash advance - we'll pay you first.</p></button>
        </div>
      </div>
