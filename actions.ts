...
export async function cancelExpense(formData: FormData) {
  ...
  const wasReturnedForApproval = expense.decisionNote === 'Previous approval was returned for approval again.';
  const canCancel = expense.status === 'PENDING' || expense.status === 'AWAITING_PURCHASE' || (expense.status === 'READY_TO_PAY' && wasReturnedForApproval);
  if (!canCancel) throw new Error('This expense can no longer be cancelled.');
